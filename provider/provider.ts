...
export abstract class Provider {
	...
	async dispatch(db: DbContext) {
		const host = await db.host.first(host => host.provider.valueOf() == this.name);
		...
		for (let event of events) {
			...
			const existing = await db.event.first(item => item.hash.valueOf() == event.hash);
	...
	static async update(db: DbContext) {
		for (let provider of this.findProviders()) {
			const host = await db.host.first(host => host.provider.valueOf() == provider.name);
		...
		for (let path of readdirSync(join(__dirname, 'providers'))) {
			if (path[0] != '.' && path.endsWith('.js')) {
				...
				try {
					...
					console.log('Invalid provider at', path);
				}
			}
		}
